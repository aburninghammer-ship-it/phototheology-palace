...
BEGIN
  ...
  IF v_church_id IS NOT NULL THEN
    ...
    -- Map church tier to a valid profile subscription_tier
    -- The check constraint only allows: 'free', 'essential', 'premium', 'student'
    v_profile_tier := CASE
      WHEN v_church_tier IN ('free', 'essential', 'premium', 'student') THEN v_church_tier
      ELSE 'premium'
    END;
