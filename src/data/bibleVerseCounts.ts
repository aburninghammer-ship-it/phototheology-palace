// Complete KJV verse counts for all 1,189 chapters
// Each array contains verse counts for each chapter of that book
...
export function getVerseCountForChapter(bookCode: string, chapter: number): number {
  const bookVerses = BIBLE_VERSE_COUNTS[bookCode];
  if (!bookVerses || chapter < 1 || chapter > bookVerses.length) {
    return 0;
  }
  return bookVerses[chapter - 1];
}

// Get total verses for a book
export function getTotalVersesForBook(bookCode: string): number {
  const bookVerses = BIBLE_VERSE_COUNTS[bookCode];
  if (!bookVerses) return 0;
  return bookVerses.reduce((sum, count) => sum + count, 0);
}
