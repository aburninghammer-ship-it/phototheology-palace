...
import {
  MessageCircle, BookOpen, Church, Settings, Layers,
  Gamepad2, GraduationCap, Headphones,
} from "lucide-react";
import { useNavigate } from "react-router-dom";
import { useState } from "react";

export type ExplorerTab =
  | "chat" | "bible" | "chapel" | "study" | "listen"
...
const TABS: TabItem[] = [
  ...
  },
  {
    ...
    activeBorder: "border border-orange-400/30",
    activeGlow: "shadow-[0_0_15px_rgba(251,146,60,0.25)]",
    activeText: "text-orange-300",
    hoverBg: "hover:bg-orange-500/10",
    iconGlow: "drop-shadow-[0_0_6px_rgba(251,146,60,0.6)]",
  },
  {
    id: "games-space",
    label: "Games",
...
export function ExplorerModeSidebar({ activeTab, onTabChange }: ExplorerModeSidebarProps) {
  ...
  const renderDesktopTab = (tab: TabItem) => {
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 py-1 px-2 space-y-1.5 overflow-y-auto">
          {spaceTabs.map(renderDesktopTab)}
        </nav>
      ...
      >
        <div className="flex items-center justify-evenly h-[56px] px-0.5">
          {TABS.map((tab) => {
            const isActive = activeTab === tab.id;
            const Icon = tab.icon;
            return (
              <button
                ...
                onClick={() => onTabChange(tab.id)}
                className={cn(
                  "flex flex-col items-center justify-center gap-0.5 px-1.5 py-1.5 rounded-xl transition-all min-w-0 flex-1 active:scale-95",
                  isActive
